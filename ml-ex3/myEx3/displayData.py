...
def displayData(X, example_width=None):
    ...
    if example_width is None:
        example_width = (int)(np.round(np.sqrt(X.shape[1])))  # 每一张图片的宽度
    ...
    display_rows = (int)(np.floor(np.sqrt(m)))  # 展示的图片一共排成几行
    display_cols = (int)(np.ceil(m / display_rows))  # 几列
    ...
    # Copy each example into a patch on the display array  这里没有看懂
    curr_ex = 0
